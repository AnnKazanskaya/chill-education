...
/* Отправка результата теста учителю: подробный отчёт + голосовое (в группу «Заявки») */
async function sendTestResults({ name, contact, level, answers, speaking }){
  try{
    const total = answers.length;
    const correct = answers.filter(a=>a.correct).length;
    const skillLine = ["gv","reading","listening"].map(sk=>{
      const at = answers.filter(a=>a.item.skill===sk); const c = at.filter(a=>a.correct).length;
      return SKILL_LABEL[sk] + ": " + c + "/" + at.length;
    }).join("\n");
    const qLines = answers.map(a=>{
      const it=a.item; const chosen = it.opts[a.chosen];
      const base = (a.correct?"✅":"❌") + " [" + LEVEL_INFO[it.level].code + "] " + it.stem + " → " + chosen;
      return a.correct ? base : base + "  (верно: " + it.opts[it.answer] + ")";
    }).join("\n");
    const spLine = !speaking ? "не записано" : speaking.audioUrl ? "🎙 голосовое — файл ниже" : (speaking.text ? "✍️ " + speaking.text : "не записано");
    let text =
      "📝 Результат теста на уровень\n" +
      "👤 Ученик: " + (name||"—") + "\n" +
      "📞 Контакт: " + (contact||"—") + "\n" +
      "🎯 Уровень: " + level + " · верно " + correct + "/" + total + "\n" +
      (utmLines() ? utmLines() + "\n" : "") +
      "\n📊 " + skillLine +
    ...
    if(speaking && (speaking.audioBlob || speaking.audioUrl)){
      try{
        const blob = speaking.audioBlob || await (await fetch(speaking.audioUrl)).blob();
        const m = speaking.mime || blob.type || "";
        const ext = (m.indexOf("mp4")>=0||m.indexOf("m4a")>=0) ? "m4a" : m.indexOf("ogg")>=0 ? "ogg" : m.indexOf("wav")>=0 ? "wav" : "webm";
...
@media (min-width:761px){.elc-hero .elc-facts{display:grid;grid-template-columns:repeat(2,auto);justify-content:start}.elc-hero .elc-fact{white-space:nowrap;font-size:13px;padding:8px 13px}}
...
@media (max-width:760px){
  ...
  .elc-ben-float{display:block;float:right;width:148px;margin:-34px -14px 8px 10px;animation:elc-float 6s ease-in-out infinite}
  .elc-hero .elc-h1{font-size:25px;margin:8px 0 10px}
...
const LISTEN_POOL = {
  ...
  upper: [
    ...
    { id:"ls-up-2", rate:1.0, file:"audio/ls-up-2.mp3", audio:"Honestly, I used to be quite sceptical about online learning. I assumed you'd lose all the interaction you get in a classroom. But having taught a few courses remotely now, I've come round to it. The tools have improved enormously, and in some ways students participate more — the quieter ones, especially, seem more willing to speak up. That said, it does take a lot more preparation than people realise.",
      q:[
...
function ListeningPlayer({ audio, rate, file }){
  ...
  useEffect(()=>{
    if(file){
      ...
      el.onerror = ()=>{ audioRef.current = null; }; // файл не загрузился — откатимся на синтез
      audioRef.current = el;
      return ()=>{ try{ el.pause(); }catch(e){} audioRef.current = null; };
    }
  ...
  const play = ()=>{
    if(audioRef.current){
      ...
      try{ el.currentTime = 0; el.play().then(()=>setStatus("playing")).catch(()=>{ audioRef.current=null; play(); }); return; }catch(e){ audioRef.current=null; }
    }
    try{
      ...
      u.onerror=()=>{ setStatus("idle"); setSupported(false); };
      setStatus("playing"); synth.speak(u);
    }catch(e){ setSupported(false); setStatus("idle"); }
  };
  const toggle = ()=>{
    if(audioRef.current){
      const el = audioRef.current;
      if(status==="playing"){ el.pause(); setStatus("paused"); }
      else if(status==="paused"){ el.play().then(()=>setStatus("playing")).catch(()=>{}); }
      else { play(); }
      return;
    }
    try{
  ...
  const isPlaying = status==="playing", isPaused = status==="paused";
  return (
    <div className={"elc-listen"+(isPlaying?" playing":"")}>
      <div className="elc-listen-row">
        <button className="elc-play" onClick={toggle} aria-label={isPlaying?"Пауза":isPaused?"Продолжить":"Прослушать аудио"}>
          {isPlaying ? <PauseIcon/> : <PlayIcon/>}
        </button>
        <div className="elc-lm">
          <div className="elc-lt">{isPlaying ? "Идёт воспроизведение… нажми, чтобы поставить на паузу" : isPaused ? "Пауза — нажми, чтобы продолжить" : "Нажми, чтобы прослушать"}</div>
...
function SpeakingBlock({ prompt, onComplete }){
  ...
  const blobRef = useRef(null); // сам файл записи — живёт дольше компонента, нужен для отправки в Telegram
  const stopWaitRef = useRef(null); // ожидание файла, если тест завершили не остановив запись
  ...
  useEffect(()=>()=>{ // cleanup (ссылку не отзываем: она нужна плееру в отчёте и отправке после записи на урок)
  ...
  const startRec = async ()=>{
    ...
    try{
      ...
      mr.onstop = ()=>{
        ...
        if(stopWaitRef.current){ stopWaitRef.current(blob); stopWaitRef.current = null; }
      };
      mr.start();
  ...
  const reRec = ()=>{ try{ audioUrl && URL.revokeObjectURL(audioUrl); }catch(e){} blobRef.current=null; setAudioUrl(null); setSeconds(0); setStatus("idle"); };

  const hasAnswer = !!audioUrl || (status==="fallback" && text.trim().length>0);
  const finish = async ()=>{
    /* если человек жмёт «Узнать результат» прямо во время записи —
       сами останавливаем запись и ждём, пока файл сформируется */
    let blob = blobRef.current;
    if(status==="recording"){
      blob = await new Promise(res=>{
        stopWaitRef.current = res;
        setTimeout(()=>{ if(stopWaitRef.current){ stopWaitRef.current=null; res(blobRef.current); } }, 3000);
        stopRec();
      });
    }
    onComplete({ audioBlob: blob || blobRef.current, audioUrl, mime, seconds, text: text.trim(), prompt: prompt.prompt, level: prompt.level });
  };
        ...
        <div className={"elc-card"+(phase==="intro"?" elc-card-open":"")}>
          ...
          {phase==="intro" && (
            <div className="elc-q elc-hero">
              <div>
                <img className="elc-ben-float" src="img/cat-test-trim.png?v=1" alt="" aria-hidden="true" />
                <div className="elc-eyebrow">Тест на уровень английского · <b style={{whiteSpace:"nowrap"}}>шкала CEFR</b></div>
              ...
              <div className="elc-hero-visual" aria-hidden="true">
                <img className="elc-hv-ben" src="img/bigben.png?v=1" alt="" />
              </div>
            </div>
          )}
          ...
          {currentItem && (phase==="router"||phase==="core"||phase==="listen") && (
            <div className="elc-q" key={currentItem.id}>
              ...
              {currentItem.skill==="listening" && <ListeningPlayer audio={listenClip.audio} rate={listenClip.rate} file={listenClip.file} />}
        ...
        {booked ? (
          ...
          <form className="elc-leadform" onSubmit={async (e)=>{
            ...
            try{
              ...
              await Promise.race([
                sendTestResults({ name: (lead.name.trim()||name||""), contact: lead.contact.trim()+(lead.messenger?" · "+lead.messenger:""), level: info.code, answers, speaking }),
            ...
            <select className="elc-cta-inp" name="messenger" required value={lead.messenger||""} onChange={e=>setLead(l=>({...l,messenger:e.target.value}))} style={{cursor:"pointer",color:lead.messenger?undefined:"#8C90AC"}}>
              <option value="" disabled>В какой соцсети тебя можно найти по этому номеру?</option>
              <option>Telegram</option><option>WhatsApp</option><option>Telegram и WhatsApp</option>
              <option value="Ни в какой">Ни в какой</option>
            </select>
            <label style={{display:"flex",alignItems:"flex-start",gap:"8px",fontFamily:"var(--font-body)",fontSize:"12px",lineHeight:1.4,color:"rgba(255,255,255,.75)",cursor:"pointer"}}>
